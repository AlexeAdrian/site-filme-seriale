  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh' }}>
        <CircularProgress />
        <Typography sx={{ marginLeft: 2 }}>Loading series details...</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ backgroundColor: '#1e293b', color: 'white', padding: '20px' }}>
      ...
      <Card sx={{ maxWidth: 900, margin: '20px auto', padding: 2 }}>
        ...
        />
        <Typography variant="body1" sx={{ marginTop: 2 }}>{serie.overview}</Typography>
        ...
        {similarSeries.length > 0 && (
          <Box sx={{ marginTop: 3 }}>
            <Typography variant="h6" gutterBottom>Similar Series:</Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
              {similarSeries.map((similarSerie) => (
                <Card key={similarSerie.id} sx={{ width: 200, cursor: 'pointer', borderRadius: '15px' }}>
                  <CardMedia
                    component="img"
                    height="300"
